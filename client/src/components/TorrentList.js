...
function TorrentActions({ link, toast }) {
  ...
  const generateProxyUrl = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/unrestrict-link`, {
        method: 'POST',
        headers: {
...
function TorrentList() {
  ...
  const fetchTorrents = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${process.env.REACT_APP_API_URL}/torrents`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  return (
    <VStack spacing={4} width="100%" align="stretch">
      <HStack justify="space-between">
        <Text fontSize="xl" fontWeight="bold">Current Torrents</Text>
        <Button
        ...
        <Table variant="simple">
          ...
          <Tbody>
            {torrents.map((torrent) => (
              <Tr key={torrent.id}>
                <Td>
                  <Tooltip label={torrent.filename} placement="top">
                ...
                <Td>
                  <Progress
                    ...
                    size="sm"
                    colorScheme={torrent.status === 'error' ? 'red' : 'blue'}
                  />
                  <Text fontSize="xs" mt={1}>{torrent.progress}%</Text>
                </Td>
                ...
                <Td>
                    {torrent.status === 'downloaded' && torrent.links && (
                        <TorrentActions link={torrent.links[0]} toast={toast} />
                    )}
                </Td>
              </Tr>
            ))}
